...
export const GameManager: React.FC = () => {
  ...
  const [status, setStatus] = useState<'GUESSING' | 'CORRECT' | 'REVEALED'>(
    'GUESSING'
  )
  ...
  const fetchPuzzle = async () => {
    ...
    try {
      ...
      setFeedback(
        'Failed to connect to Puzzle API. Make sure the server is running.'
      )
    } finally {
      setLoading(false)
    }
  ...
  return (
    <div className="game-container">
      <div className="puzzle-section">
        ...
        <div
          className={`status-message ${status === 'CORRECT' ? 'correct' : 'incorrect'}`}
        >
          {feedback}
        </div>
      </div>
